...
// Remove the juice: normalize so both sides add to 100%
function removeJuice(oddsA, oddsB) {
  const rawA = americanToRawProb(oddsA);
  const rawB = americanToRawProb(oddsB);
  const total = rawA + rawB; // This is always > 1.0 due to juice
  // Divide by total to get fair probabilities
  return {
    ...
    for (const event of data) {
      ...
      for (const bookmaker of (event.bookmakers || [])) {
        ...
        const displayName = bookmaker.key === 'draftkings' ? 'DraftKings' :
          bookmaker.key === 'fanduel' ? 'FanDuel' : bookmaker.key;
        
        // Remove juice before converting
        const fair = removeJuice(h2h.outcomes[0].price, h2h.outcomes[1].price);
        
        bookPrices[displayName] = {
          teamA: h2h.outcomes[0].name,
          teamB: h2h.outcomes[1].name,
          priceA: fair.probA,
          priceB: fair.probB,
          rawOddsA: h2h.outcomes[0].price,
          rawOddsB: h2h.outcomes[1].price,
        };
      }
...
export async function fetchAllOdds() {
  ...
  for (const [sport, key] of Object.entries(SPORT_MAP)) {
    console.log(`  Fetching ${sport} sportsbook odds...`);
