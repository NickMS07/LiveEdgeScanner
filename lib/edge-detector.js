    ...
    for (const odds of oddsApiMarkets) {
      ...
      if (homeMatches || awayMatches) {
        for (const [bookName, bookData] of Object.entries(odds.bookPrices)) {
          if (teamsMatch(bookData.teamA, poly.teamA)) combined.prices[bookName] = [bookData.priceA, bookData.priceB];
          else if (teamsMatch(bookData.teamB, poly.teamA)) combined.prices[bookName] = [bookData.priceB, bookData.priceA];
        }
...
export function calculateEdge(market) {
  ...
  let edgeClass = 'none';
  if (bestGap >= thresholds.strong) edgeClass = 'strong';
  else if (bestGap >= thresholds.moderate) edgeClass = 'moderate';
  else if (bestGap >= thresholds.weak) edgeClass = 'weak';
  return { gap: bestGap, side: bestSide, betTeam, platform: bestPlatform,
    polyPrice, bookPrice: polyPrice + bestGap, edgeClass };
}
