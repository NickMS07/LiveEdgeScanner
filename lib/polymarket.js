...
export async function fetchPolymarketSports() {
  ...
  try {
    ...
    for (const market of data) {
      // Parse outcomes - always a JSON string from this API
      let outcomes;
      try {
      ...
      // Parse prices - always a JSON string from this API
      let rawPrices;
      try {
        rawPrices = typeof market.outcomePrices === 'string' ? JSON.parse(market.outcomePrices) : market.outcomePrices;
      } catch { continue; }
      if (!rawPrices || !Array.isArray(rawPrices) || rawPrices.length < 2) continue;
      
      const prices = rawPrices.map(p => Math.round(parseFloat(p) * 100));
      if (prices.some(p => isNaN(p))) continue;
      ...
      markets.push({
        ...
        teamA: String(outcomes[0]),
        teamB: String(outcomes[1]),
        priceA: prices[0],
        priceB: prices[1],
